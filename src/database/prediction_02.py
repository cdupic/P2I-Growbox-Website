...
from src.database.measure import get_sensor_type, get_sensor_id
			...
			for date in dates:
				...
				if day not in data_night:
					...
					if data_measures!=(None, None, None):
						data_night[day] = {'min': data_measures[1]/10, 'max': data_measures[0]/10, 'avg': data_measures[2]/10}
					else :
				...
				if day not in data_day:
					...
					data_measures = get_min_max_avg_o2(sensor_id, day_start, day_start + timedelta(hours=12))
					if data_measures!=(None, None, None):
						data_day[day] = {'min': data_measures[1]/10, 'max': data_measures[0]/10, 'avg': data_measures[2]/10}
					else :
