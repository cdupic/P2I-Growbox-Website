...
# add the relations in GreenHousePlants and update them if they already exist
        for plant_id in list_plants_id:
            ...
            plant_exist = False
            cursor.execute("SELECT plant_id FROM GreenHousePlants WHERE EXISTS (SELECT plant_id FROM GreenHouses WHERE "
                           "plant_id = %s and greenhouse_serial = %s )"
                           , (plant_id, greenhouse_serial,))
            for (id_plant,) in cursor:
                if id_plant:
                    plant_exist = True
            ...
            if not plant_exist:
                ...
                cursor.execute("UPDATE GreenHousePlants SET date_start = NOW() "
                               "WHERE plant_id = %s and greenhouse_serial = %s ",
                    (plant_id, greenhouse_serial))
                db.commit()
...
# check if there are plants that are not in the list anymore :-> update the date_fin
        cursor.execute("SELECT plant_id, date_end "
                        "FROM GreenHousePlants WHERE greenhouse_serial = %s", (greenhouse_serial,))
        for (plant_id, date_fin) in cursor:
            if plant_id not in list_plants_id and date_fin is None:
